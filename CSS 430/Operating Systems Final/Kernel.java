...
public class Kernel
{
    ...
    // //suppress warnings for compiling to avoid -Xlint:warning 
    // Kernel.java:267: warning: [unchecked] unchecked call to getConstructor(Class<?>...) as a member of the raw type Class
    ...
    private static int sysExec( String args[] ) {
		...
		try {
			...
			if ((tcb = scheduler.addThread(t)) == null)
			{
				return ERROR;
			}
			return tcb.getTid();
